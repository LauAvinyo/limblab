...
from packages.limblab.limblab.pipeline import _create_experiment
from packages.limblab.tools import (
    _clean_volume,
    _extract_surface,
    _morph_limb,
    _rotate_limb,
...
from packages.limblab.visualizations import (
    dynamic_slab,
...
from packages.limblab.limblab.models import Experiment, Channel

app = typer.Typer()

# Help text for experiment folder argument
EXPERIMENT_FOLDER_HELP = (
    "Path to the experiment folder containing pipeline.log and processed data"
)
...
def create_experiment(
    experiment_name: str, experiment_folder_path: Optional[str] = None
):
    """
...
def clean_volume(
    ...
    volume_path: Path,
    channel_name: str,
    gaussian_sigma: Optional[str] = typer.Option(
        None,
        "--sigma",
        help="Gaussian smoothing parameters as 'x,y,z' (default: '6,6,6')",
    ),
    frequency_cutoff: Optional[float] = typer.Option(
        None, "--cutoff", help="Frequency cutoff for low-pass filtering (default: 0.05)"
    ),
    low_res_size: Optional[str] = typer.Option(
        None, "--size", help="Output volume size as 'x,y,z' (default: '512,512,296')"
    ),
):
    """
    ...
    _clean_volume(
        ...
        volume_path,
        channel_name,
        gaussian_sigma=parsed_sigma,
        frequency_cutoff=frequency_cutoff,
        low_res_size=parsed_size,
    )
...
def extract_surface(
    ...
    auto: bool = typer.Option(
        False, help="Automatically determine isovalue from volume histogram"
    ),
):
    """
    Extract 3D surface mesh from volume data.
...
def align(
    ...
    morph: bool = typer.Option(
        False, help="Perform non-linear morphing instead of rotation"
    ),
):
    """
...
@app.command()
def vis(algorithm: VisAlgorithm, experiment_folder_path: Path, channels: List[str]):
    """
    ...
    if algorithm == VisAlgorithm.raycast:
        if len(channels) > 1:
            print(f"WARNING: Raycast only uses one channel. Using {channels[0]}")
