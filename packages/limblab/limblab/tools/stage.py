        ...
        if event.keypress == "s":
            if plt.line:
                ...
                if response.status_code != 200:
                    raise ConnectionError(f"Staging server returned status code {response.status_code}.")
...
def stage_limb(experiment: Experiment) -> int:
    ...
    if surface_name is None or base is None:
        raise ValueError("Surface name and base path must be provided in the experiment object.")
